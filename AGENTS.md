...
| **독립 파이프라인** | 빌드된 시스템 위에 추가되는 분리된 분석 워크플로우 | 기존 코드 무수정으로 축적 데이터에 대한 2차 분석이 필요할 때. Producer-Consumer 디커플링 필수 |
...
축적된 크롤링+분석 데이터에 대한 빅데이터 통찰 보고서를 생성하는 스킬. Workflow B(Insight Pipeline)의 M1-M7 모듈 결과를 기반으로 구조적 분석 보고서를 작성한다.

- **트리거**: "통찰 보고서 작성해줘", "미래통찰 보고서", "인사이트 분석"
- **진입점**: `.claude/skills/insight-report/SKILL.md`
- **핵심**: 7개 분석 모듈(교차언어, 내러티브, 엔티티, 시간패턴, 지정학, 경제, 종합) × 27개 지표. P1 결정론적 Python (92%+)
